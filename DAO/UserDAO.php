<?php 
    ...
    class UserDAO {
        ...
        public function modify($id, User $newUser) {
            try
            {
                ...
                $parameters["user_name"] = $newUser->getName();
                $parameters["user_last_name"] = $newUser->getLastName();
                $parameters["email"] = $newUser->getEmail();
                $parameters["user_password"] = $newUser->getPassword();
                $parameters["birth_date"] = $newUser->getBirthDate();
        ...
        public function getUser($email, $password) {
            try {
                ...
                $query = "SELECT * FROM " . $this->tableName . " WHERE (email = :email && user_password = :password);";
        ...
        public function mapear($value) {
            ...
            $resp = array_map(function($p) {
                return new User ($p['user_role'],$p['user_id'],$p['user_name'],$p['user_last_name'],$p['email'],$p['user_password'],$p['birth_date']);
            }, $value);
